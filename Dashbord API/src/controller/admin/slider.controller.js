...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_record = await sliderModal.find(filter).countDocuments()

        await sliderModal.find(filter).select('name  image status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
