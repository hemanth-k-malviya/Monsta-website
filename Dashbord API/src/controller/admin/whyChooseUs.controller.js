...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_record = await whyChooseUsModal.find(filter).countDocuments()

        await whyChooseUsModal.find(filter).select('name image status order description').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.update = async (request, response) => {
    try {
        ...
        await whyChooseUsModal.updateOne({
...
exports.destroy = async (request, response) => {
    try {
        ...
        await whyChooseUsModal.updateMany({
