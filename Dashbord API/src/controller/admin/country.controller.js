const countryModal = require('../../models/country.js');
...
exports.create =async (request, response) => {
    ...
    try {
        ...
        var saveData =await new countryModal(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        total_record = await countryModal.find(filter).countDocuments()

        await countryModal.find(filter).select('name code status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
