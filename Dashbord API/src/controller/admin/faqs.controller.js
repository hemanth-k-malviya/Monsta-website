const faqsModal = require('../../models/faqs.js');
...
exports.create =async (request, response) => {
    ...
    try {
        ...
        var saveData =await new faqsModal(data).save()
            .then((result) => {
                const data = {
                    _status: true,
...
exports.view = async (request, response) => {
    ...
    try {
        ...
        const addCondition = [
            {
                deleted_at: null,
                question: { $exists: true } // only show records where question exists in db
            }
        ];
        ...
        //     //         var order = new RegExp(request.body.order, "i");
        //     //         addCondition.push({ order: order })
        //     //     }
        //     // }
        ...
        total_record = await faqsModal.find(filter).countDocuments()

        await faqsModal.find(filter).select('question answer status order').skip(skip).limit(limit).sort({ _id: 'desc' })
            .then((result) => {
                if (result.length > 0) {
...
exports.details = async (request, response) => {
    ...
    try {
        ...
        await faqsModal.findById(request.params.id)
            .then((result) => {
                if (result) {
                    const data = {
                        _status: true,
